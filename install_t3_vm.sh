...
function upload_workdir() {
  ...
  gcloud compute tpus tpu-vm scp myprojectdir.zip $GCP_v3_TPU_NAME: \
    --worker=all \
    --zone=$GCP_ZONE_v3
  rm myprojectdir.zip
  gcloud compute tpus tpu-vm ssh $GCP_v3_TPU_NAME \
    --zone $GCP_ZONE_v3 \
    --project $GCP_PROJECT \
    --worker all \
...
function copy_file() {
  SRC=$1
  DST=$2
  gcloud compute tpus tpu-vm scp "${SRC}" "${GCP_v3_TPU_NAME}:${2}" \
    --worker=all \
    --zone=$GCP_ZONE_v3
}
...
run_command "pip install jax[tpu]==0.4.29 -f https://storage.googleapis.com/jax-releases/libtpu_releases.html"
